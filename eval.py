...
def create_report(test, baseline, sample_rollouts, costs, num_segs):
  ...
  res.append(f"<h3 style='font-size: 30px;'><span style='background: {COLORS['test']}; color: #fff; padding: 10px'>Test Controller: {test}</span> vs <span style='background: {COLORS['baseline']}; color: #fff; padding: 10px'>Baseline Controller: {baseline}</span></h3>")
  ...
  if passed_baseline:
    res.append(f"<h3 style='font-size: 20px; color: #27ae60'> [PASS] Test Controller ({test}) passed Baseline Controller ({baseline})! [PASS] </h3>")
    ...
    res.append(f"<h3 style='font-size: 20px; color: #c0392b'> [FAIL] Test Controller ({test}) failed to beat Baseline Controller ({baseline})! [FAIL]</h3>")
